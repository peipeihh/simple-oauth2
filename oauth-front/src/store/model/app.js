...
const actions = {
    ...
    login({commit, dispatch}, data){
        ...
        api.accountService.doLogin({"token": token}).then(function (resp) {
    ...
    registerAccount({commit, dispatch}, data){
        api.accountService.registerAccount(data).then(function (resp) {
            dispatch("displayPromptByResponseMsg", resp);
        }.bind(this)).catch(function (err) {
            dispatch("displayPromptByResponseMsg", err.response);
        }.bind(this))
    ...
    fetchAccount({commit, dispatch}, data){
        api.accountService.fetchMyAccount(data).then(function (resp) {
            dispatch("displayPromptByResponseMsg", resp);
        }.bind(this)).catch(function (err) {
            dispatch("displayPromptByResponseMsg", err.response);
        }.bind(this))
    ...
    setUserName({commit, dispatch}, data){
        api.accountService.saveAccountUserName(data).then(function (resp) {
            if (resp.data != null && resp.data.code != null && resp.data.code >= 0) {
                let token = resp.data.details;
                jwtTokenUtil.save(token);
                dispatch("initLoginInfo");
                let response = {status: 200, data: {code: resp.data.code, message: resp.data.message, details: "用户名设置成功。"}};
    ...
    setUserPassword({commit, dispatch}, data){
        let token = encryptor.generateToken(data);
        api.accountService.saveAccountPassword({"token": token}).then(function (resp) {
            dispatch("displayPromptByResponseMsg", resp);
        }.bind(this)).catch(function (err) {
            dispatch("displayPromptByResponseMsg", err.response);
        }.bind(this))
